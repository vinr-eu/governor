...
export const rdsPasswordSecret: SecretPlugin = {
  id: "rds-password",
  usage: "<db-identifier> <db-user> [--profile name] [--password value]",
  ...
  async store(args, flags) {
    ...
    if (!dbIdentifier || !dbUser) {
      logger.error(
        `Usage: governor store rds-password ${rdsPasswordSecret.usage}`,
      );
      process.exitCode = 1;
      return;
    }
