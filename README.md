...
| `aws_rds_instance_query`         | Runs one SQL statement against an RDS/Aurora database — over an SSH tunnel through a bastion, or directly if it's publicly accessible — authenticated with a short-lived IAM DB token by default, or a stored password (`governor store rds-password`) as an opt-in.                                                                                                 |
| `aws_elasticache_redis_command`  | Runs one Redis command against a Valkey/Redis OSS ElastiCache replication group or cache cluster — over an SSH tunnel through a bastion, or directly if governor already has VPC access (ElastiCache has no public-endpoint mode). Uses a stored AUTH token (`governor store redis-auth-token`) when the resource requires one. Memcached clusters aren't supported. |
...
Auth method: connection string — one MongoDB URI per cluster, stored under a nickname via `governor store
mongodb-uri <cluster-name>`, credentials embedded. There's no `governor setup mongodb` step, since (unlike AWS)
there's no account-wide credential underneath it.

MCP tools:

| Tool            | Does                                                                                                                                                                                                                                                                           |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `mongodb_query` | Runs one `find` filter or `aggregate` pipeline against a database/collection on a cluster — over an SSH tunnel through a bastion (`governor store mongodb-bastion-key`), or directly using the stored URI as-is (the common case for Atlas). Read-only, no write escape hatch. |
