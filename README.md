...
| `governor store <secret-type>` | Store an auxiliary secret a tool opts into, e.g. `governor store ssh-key <bastionName> --user <name> --key-file <path>`, `governor store rds-password <name> <dbUser>`, or `governor store redis-auth-token <name>`. See [docs/tools/rds.md](docs/tools/rds.md) / [docs/tools/elasticache.md](docs/tools/elasticache.md). |
